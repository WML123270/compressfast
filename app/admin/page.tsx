...
export default function AdminDashboardPage() {
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <h2 className="text-lg font-semibold text-slate-700 mt-4">最近购买</h2>
            ...
            <tbody>
              {(stats.recentPurchases ?? []).map((p: any, i: number) => (
                <tr key={i} className="border-b border-slate-50">
                  ...
                  <td className="px-4 py-3 text-slate-400 text-xs">{new Date(p.created_at).toLocaleDateString('zh-CN')}</td>
                  <td className="px-4 py-3 text-slate-600 text-xs">{p.orderAmount || '$24.99'}</td>
                </tr>
              ))}
            </tbody>
          </table>
