        ...
        <div className="px-4 pb-4 space-y-3 border-slate-700 pt-3">
          ...
          {watermark.enabled && (
            <>
              ...
              {watermark.type === 'text' && (
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <div>
                      <label className="text-slate-400">{isZh ? '字号' : 'Size'}</label>
                      <input
                      ...
                      />
                      <span className="text-slate-400">{watermark.fontSize}px</span>
                    </div>
                    <div>
                      <label className="text-slate-400">{isZh ? '透明度' : 'Opacity'}</label>
                      <input
                        ...
                        className="w-full h-1.5 accent-brand-600"
                      />
                      <span className="text-slate-400">{Math.round(watermark.fontOpacity * 100)}%</span>
                    </div>
                    <div>
                      <label className="text-slate-400">{isZh ? '旋转' : 'Rotate'}</label>
                      <input
                  ...
                  <div>
                    <label className="text-slate-400 block mb-1">{isZh ? '颜色' : 'Color'}</label>
                    <input
                      type="color"
                  ...
                  {watermark.imageDataUrl ? (
                    <div className="flex items-center gap-3 p-3 rounded-lg bg-slate-800/50 border border-slate-700">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-slate-400 truncate">{isZh ? '水印图片已加载' : 'Watermark loaded'}</p>
                      </div>
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 px-4 py-3 text-slate-400 border-slate-600 rounded-lg hover:border-brand-400 hover:text-brand-600 transition-colors"
                    >
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full h-1.5 accent-brand-600"
                      />
                      <span className="text-slate-400">{Math.round(watermark.imageOpacity * 100)}%</span>
                    </div>
                    <div>
                      <label className="text-slate-400">{isZh ? '大小' : 'Scale'}</label>
                      <input
                        type="range" min={5} max={100}
                        ...
                        className="w-full h-1.5 accent-brand-600"
                      />
                      <span className="text-slate-400">{Math.round(watermark.imageScale * 100)}%</span>
                    </div>
                  </div>
                </div>
              )}
              ...
              {/* Position grid */}
              <div>
                <label className="text-slate-400 block mb-1.5">{isZh ? '位置' : 'Position'}</label>
                    ...
                    <button
                      ...
                      className={`w-7 h-7 flex items-center justify-center rounded text-sm transition-colors ${
                        watermark.position === p.value
                          ? 'bg-brand-900/50 text-brand-300 ring-1 ring-brand-300'
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-slate-400">{isZh ? '水平边距(px)' : 'Margin X (px)'}</label>
                  <input
                    type="number" min={0} max={200}
                    value={watermark.marginX}
                    onChange={(e) => setWatermark({ marginX: Math.max(0, Number(e.target.value) || 0) })}
                    className="w-full px-2 py-1 rounded border border-slate-600 bg-slate-700 text-slate-100 outline-none focus:border-brand-500"
                  />
                </div>
                <div>
                  <label className="text-slate-400">{isZh ? '垂直边距(px)' : 'Margin Y (px)'}</label>
                  <input
                    type="number" min={0} max={200}
